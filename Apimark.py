...
class Listing(Resource):
    ...
    def get(self, listing_id, user_id=0):
        ...
        user_history = users.find_one({"_id": user_id})
        l = []
        for i in user_history["keywords"]:
            l.append(i)

        l.append(result["keyword"])
        users.update_one({"_id": user_id}, {"$set": {"keywords": l}})
...
class Suggestions(Resource):
    ...
    def get(self, user_id):
        ...
        for i in user["keywords"]:
            result = list(listings.find({"keyword": i}))
            recommend.append(result)
        json_docs = [json.dumps(doc, default=json_util.default) for doc in recommend]
